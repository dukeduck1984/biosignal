...
class SEMG(object):
    def __init__(self, emg_data, freq=2000):
        self.emg_data = np.array(emg_data)
        self.freq = int(freq)
    ...
    def remove_offset(self):
        ...
        """
        offset_removed = self.emg_data - np.mean(self.emg_data)
        self.emg_data = offset_removed
    ...
    def rectify(self):
        ...
        """
        rectified = np.abs(self.emg_data)
        self.emg_data = rectified
        return self.emg_data

    def filter_bandpass(self, freq=None, order=4, highpass=20, lowpass=500):
        """
        带通滤波
        ...
        if freq is not None:
            self.freq = freq
        freq_nyq = self.freq / 2
        high, low = highpass / freq_nyq, lowpass / freq_nyq
        b, a = signal.butter(order, [high, low], btype='bandpass')
        data_filt = signal.filtfilt(b, a, self.emg_data)
        self.emg_data = data_filt
        return self.emg_data

    def interp_signal(self, interp_freq, freq=None,):
        """
        信号采样率插值
        ...
        if freq is not None:
            self.freq = freq
        x = np.arange(self.emg_data.size)
        y = self.emg_data[x]
        interp_x = np.linspace(0, self.emg_data.size - 1, int(self.emg_data.size * (interp_freq / self.freq)))
        interp_y = np.interp(interp_x, x, y)
        self.emg_data = interp_y
        return self.emg_data

    def filter_lowpass(self, freq=None, lowpass=3):
        """
        低通滤波
        ...
        low_filt = signal.filtfilt(b, a, self.emg_data)
        self.emg_data = low_filt
        return self.emg_data

    def calc_rms(self, window_size, freq=None, overlap='default'):
        """
        计算均方根振幅
        ...
        emg_data_squared = np.power(self.emg_data, 2)
        ...
        point_size = window_size / 1000 * self.freq
        midpoint = round(point_size / 2)
        overlap_size = overlap / 1000 * self.freq
        ...
        self.emg_data = rms_final
        return rms_final_t, self.emg_data

    def onoff_threshold(self, percentage=0.05):
        """
        计算肌电信号的On & Off阈值
        ...
        """
        peak_value = np.max(self.emg_data)
        threshold_amp = peak_value * percentage
    ...
    def power_spectrum(self):
        ...
        """
        power_spect = abs(np.fft.rfft(self.emg_data))
        fft_freq = np.fft.rfftfreq(self.emg_data.size) * 1000
...
class GasExchange(object):
    def __init__(self, breath_data, time_series=None):
        self.breath_data = breath_data
        self.time_series = time_series

    def breath_time_convert(self, raw_time_string):
        """
        ...
        self.time_series = np.array(time_series_sec)
        return self.time_series

    def breath_smooth(self, time_series=None, window_size=30, avg_by='time'):
        """
        气体代谢数据进行平滑处理
        ...
        if avg_by == 'lowpass':
            ...
            filtered_breath_data = signal.filtfilt(b, a, self.breath_data)
            # Return breath time series and smoothed breath data in a tuple, both are ndarray
            return self.time_series, filtered_breath_data
        elif avg_by == 'time':
            ...
            smoothed_block = int(self.time_series[-1] / window_size)
            ...
            for i in range(smoothed_block):
                data_list = []
                for time, data in zip(self.time_series, self.breath_data):
            ...
            midpoint = round(
                window_size / 2)  # Get midpoint of the window, eg. the 8th breath if the window is 15 breathes
            window = np.ones(window_size) / float(window_size)
            smoothed_breath_data = np.convolve(self.breath_data, window, 'valid')

            first_time_index = int(midpoint - 1)
            final_time_index = int((midpoint - 1) * -1)
            smoothed_time_series = self.time_series[first_time_index: final_time_index]
...
class ForcePlate(object):
    def __init__(self, grf, bw, freq=1000):
        self.grf = np.array(grf)
        self.bw = bw
        self.freq = int(freq)

    def force_to_velocity(self, freq=None):
        """
        通过GRF一次积分求速度
    ...
    def force_to_displacement(self, freq=None):
        """
        通过VTC积分求位移（GRF的二次积分）
        ...
        vtc = self.force_to_velocity()

        distance = vtc * 1 / self.freq
        dist_time_curve = []
...
class BloodLactate(object):
    def __init__(self, intensity_data, lactate_data):
        self.intensity_data = np.array(intensity_data).reshape(-1, 1)
        self.lactate_data = np.array(lactate_data).reshape(-1, 1)

    def find_lt_loglog_method(self, plot=False):
        """
        ...
        """
        log_intensity = np.log(self.intensity_data)  # 计算运动强度数据的log
        log_lactate = np.log(self.lactate_data)  # 计算血乳酸数据的log
    ...
    def find_lt_sds_method(self, loglog=False, plot=False):
        """
        使用标准化Dmax确定LT1 & LT2
        ...
        if loglog:  # 如果使用loglog transformation计算LT1
            lt1_intensity, lt1_lactate, lt1_index = self.find_lt_loglog_method()
        else:  # 使用La浓度初次超过0.4mM的前一级负荷作为LT1
            for index, la in enumerate(self.lactate_data):
                if index > 0:
                    if (self.lactate_data[index] - self.lactate_data[index - 1]) >= 0.4:
                        lt1_index = index - 1
                        break
            lt1_intensity = self.intensity_data[lt1_index][0]
            lt1_lactate = self.lactate_data[lt1_index][0]

        # 获得LT1点的X和Y的坐标
        lt1_x = self.intensity_data[lt1_index][0]
        lt1_y = self.lactate_data[lt1_index][0]

        # 获得血乳酸最大值那点的X和Y的坐标
        la_final_x = self.intensity_data[-1][0]
        la_final_y = self.lactate_data[-1][0]
        ...
        if lt1_index >= 2:
            x1 = self.intensity_data[lt1_index - 2][0]
            y1 = self.lactate_data[lt1_index - 2][0]
            curvefit_index = lt1_index - 2
        elif 1 <= lt1_index < 2:
            x1 = self.intensity_data[lt1_index - 1][0]
            y1 = self.lactate_data[lt1_index - 1][0]
            curvefit_index = lt1_index - 1
        else:
            # print(power[lt1_index], lactate[lt1_index])
            x1 = self.intensity_data[lt1_index][0]
            y1 = self.lactate_data[lt1_index][0]
        ...
        # 曲线拟合的X和Y数据序列
        x_train = self.intensity_data[curvefit_index:]
        y_train = self.lactate_data[curvefit_index:]
        la_curve_x = np.linspace(self.intensity_data[curvefit_index][0], x2, 300)
        ...
        if plot:
            plt.plot(self.intensity_data, self.lactate_data, '^')  # 画出负荷强度vs血乳酸的原始数据点
    ...
    def find_lt_dickhuth_method(self, plot=False):
        """
        ...
        # 计算乳酸浓度/负荷比
        lactate_eq = (self.lactate_data / self.intensity_data).reshape(-1)
        ...
        lt1_x = self.intensity_data[lt1_index][0]
        # 第一级和最后一级的X坐标
        x1 = self.intensity_data[0][0]
        x2 = self.intensity_data[-1][0]
